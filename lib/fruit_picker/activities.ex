defmodule FruitPicker.Activities do
  ...
  def picks_with_outstanding_report(%Person{} = person) do
    Pick
    |> join(:left, [p], pr in PickReport, on: p.id == pr.pick_id)
    |> where([p, pr], p.status == "completed")
    |> where([p, pr], p.lead_picker_id == ^person.id)
    |> where([p, pr], is_nil(pr) or not pr.is_complete == true)
    |> select([p, pr], p)
  ...
  def picks_with_late_outstanding_report() do
    ...
    |> where([p, pr], p.status == "completed")
    |> where([p, pr], is_nil(pr) or not pr.is_complete == true)
    |> where([pick], pick.scheduled_date < ^outstanding_date)
    |> select([p, pr], p)
    |> Pick.preload_all()
    |> Repo.all()
  end
  ...
  def count_picks_for_person_this_season(%Person{} = person) do
    ...
    count =
      ...
      |> where([pick, pp], pp.person_id == ^person.id)
      |> Repo.aggregate(:count, :id)
  ...
  def count_picks_for_person_this_month(%Person{} = person) do
    this_year = Date.utc_today().year
    this_month = Date.utc_today().month
    days_in_month = Date.days_in_month(Date.utc_today())
    season_start = Date.from_erl!({this_year, this_month, 1})
    season_end = Date.from_erl!({this_year, this_month, days_in_month})
  ...
  def person_has_more_picks_than_waitlist_threshold(%Person{} = person) do
    # count_picks_for_person_this_season(person) >= @number_picks_trigger_waitlist
    false
  end
  ...
  def requires_wait(%Pick{} = pick, %Person{} = person) do
    ...
    too_many_picks and pick_not_last_minute and not has_joined_pick
  end
