defmodule Mix.Tasks.Accounts.MarkPersonInactive do
  ...
  def update_membership_status do
    ...
    Mix.shell.info("cutoff_date: #{cutoff_date}")

    MembershipPayment
    |> join(:right, [mp], person in Person, on: mp.member_id == person.id)
    |> where([mp, person], person.role == "user" and person.membership_is_active == true and not(mp.end_date >= ^cutoff_date))
    |> or_where([mp, person], person.role == "user" and person.membership_is_active == true and is_nil(mp))
    ...
    |> Enum.each(fn person ->
      ...
      Mix.shell.info("Updated #{person.first_name} #{person.last_name} (#{person.id}) to an inactive membership")
    end)
  end
end
