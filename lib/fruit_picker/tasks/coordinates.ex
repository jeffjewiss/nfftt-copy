defmodule Mix.Tasks.Coordinates do
    ...
    case HTTPoison.get(create_url(address)) do
      {:ok, response} ->
        if response.status_code == 200 do
          ...
          Mix.shell().info("Wrong response from Mapbox (#{response.status.code})")
          nil
        end

      {:error, %HTTPoison.Error{:reason => reason}} ->
        Logger.info("Wrong response from Mapbox (#{reason})")
        Mix.shell().info("Wrong response from Mapbox (#{reason})")
        nil
    end
  end
  ...
  defp update_record(%Agency{}, record, %{"longitude" => nil, "latitude" => nil}) do
    Mix.shell().info("Could not update agency '#{record.name}', latlong is nil.")
  ...
  defp update_record(%Agency{}, record, latlong) do
    ...
    case Repo.update(changeset) do
      {:ok, _agency} ->
        ...
        Mix.shell().info("Updating agency '#{record.name}' with new coordinates.")
        :ok

      {:error, _error} ->
        Mix.shell().info("Could not update agency '#{record.name}' with new coordinates.")
  ...
  defp update_record(%EquipmentSet{}, record, %{"longitude" => nil, "latitude" => nil}) do
    Mix.shell().info("Could not update equipment set '#{record.name}', latlong is nil.")
  ...
  defp update_record(%EquipmentSet{}, record, latlong) do
    ...
    case Repo.update(changeset) do
      {:ok, _set} ->
        ...
        Mix.shell().info("Updating equipment set '#{record.name}' with new coordinates.")
        :ok

      {:error, _error} ->
        Logger.info("Could not update equipment set '#{record.name}' with new coordinates.")
        Mix.shell().info("Could not update equipment set '#{record.name}' with new coordinates.")
        nil
    end
  end

  defp update_record(%Property{}, record, %{"longitude" => nil, "latitude" => nil}) do
    Mix.shell().info("Could not update property '#{record.name}', latlong is nil.")
  ...
  defp update_record(%Property{}, record, latlong) do
    ...
    case Repo.update(changeset) do
      {:ok, _property} ->
        ...
        Mix.shell().info("Updating property with id ##{record.id} with new coordinates.")

      {:error, _error} ->
        Logger.info("Could not update property with id ##{record.id} with new coordinates.")
        Mix.shell().info("Could not update property with id ##{record.id} with new coordinates.")
    end
  end
